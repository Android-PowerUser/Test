...
class PhotoReasoningViewModel(
    ...
    fun onStopClicked() {
        ...
        if (mainActivityInstance != null) {
            Log.d(TAG, "onStopClicked: Instructing ScreenCaptureService to stop.")
            val stopServiceIntent = android.content.Intent(mainActivityInstance, com.google.ai.sample.ScreenCaptureService::class.java).apply {
                action = com.google.ai.sample.ScreenCaptureService.ACTION_STOP_CAPTURE
            }
            mainActivityInstance.startService(stopServiceIntent)
        } else {
            Log.w(TAG, "onStopClicked: MainActivity instance is null. Cannot explicitly stop ScreenCaptureService from ViewModel. Service might continue running if active.")
            // Consider using application context as a fallback if essential, though stopping a service
            // without the original context that started it can sometimes be tricky.
            // For now, rely on MainActivity.getInstance().
        }
    }
