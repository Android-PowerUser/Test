        ...
        try {
            ...
            localImageReader.setOnImageAvailableListener({ reader ->
                ...
                try {
                    ...
                    image?.close()
                    // Per-screenshot cleanup of VD and IR, MediaProjection remains.
                    this.virtualDisplay?.release()
                    this.virtualDisplay = null
                    // The reader is associated with this specific capture.
                    // It's closed here to free its resources for the next takeScreenshot call.
                    reader.close() // Close the reader passed to the listener
                    if (this.imageReader == reader) { // If it's the current class member, nullify
                        this.imageReader = null
                    }
                    Log.d(TAG, "Per-screenshot resources (VD, IR listener's reader) cleaned up.")
                }
            }, Handler(Looper.getMainLooper()))
